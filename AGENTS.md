...
- 工具权限：内置工具（`BUILTIN_TOOLS`）全部保持可用，被关闭的工具由 `tool-permission-gate.ts` 内联扩展拦截、运行时逐次弹窗，不允许从 Agent 工具集移除。只读工具（`READONLY_TOOLS`）永不弹窗；`edit`/`write` 弹窗带 diff；`bash`/`powershell`（`SHELL_TOOLS`）默认关闭、首次必弹窗，弹窗与聊天窗口都展示命令原文（折叠 + 复制 + 展开，复用 `ShellCommand`）。`tool_start` 事件：`summary`（shell 为命令原文，其余为路径/搜索词）+ `diff`（edit/write 行 diff，复用 `FileDiff`）。
...
- 改能力/修 bug 必须配 Playwright 功能用例（`e2e/tests/*.spec.ts`）验证真实行为（渲染、交互、持久化），不允许只过 typecheck/lint；纯重构也要保证现有 e2e 全绿。
