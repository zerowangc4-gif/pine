...
import { JsonlSessionRepo } from "@pine/agent";
...
export function toStorableJson<T>(value: T): T {
	return JSON.parse(JSON.stringify(value, (_key, item) => (typeof item === "number" && !Number.isFinite(item) ? null : item))) as T;
}
...
export async function appendCompactionEntry(
	session: Session<JsonlSessionMetadata>,
	compaction: { summary: string; retainedTail: AgentMessage[]; tokensBefore: number; usage?: Usage; details?: unknown },
): Promise<void> {
