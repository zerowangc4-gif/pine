...
import type { PineSubscribe } from "@pine/socket-server";
import { type CompactionFold, foldedMessages, isFoldApplicable, runCompaction } from "../domain/compaction.ts";
import { toLibraryMessages } from "../domain/conformance.ts";
import { createModelRuntime, type ModelRuntime } from "../domain/model.ts";
import {
...
import { buildTools, summarizeToolCall, toolResultText } from "../domain/tools.ts";
import { resolveWorkspace, validateWorkspace } from "../domain/workspace.ts";
...
export interface AgentSessionOptions {
	...
	config: AgentConfig;
	store?: SessionStore;
	/** Fans out to every client watching this session's room (`subscribe.*` = socket-server contract). */
	subscribe: PineSubscribe;
...
export class AgentSession {
	...
	private readonly subscribe: PineSubscribe;
	...
	private async initialize(resumeSessionId?: string): Promise<void> {
		...
		this.agent = new Agent({
			...
			streamFn: (model, context, streamOptions) =>
				this.modelRuntime.models.streamSimple(model, context, streamOptions),
	...
	private async openPersistence(
		resumeSessionId?: string,
	): Promise<{ messages: AgentMessage[]; thinkingLevel?: ThinkingLevel; activeToolNames?: string[] } | undefined> {
	...
	private async onAgentEvent(event: AgentEvent, _signal: AbortSignal): Promise<void> {
		this.eventSeq += 1;
		this.subscribe.agentEvent({ sessionId: this.id, seq: this.eventSeq, event });
	...
	private onPayload(payload: unknown): undefined {
		if (this.config.debugPayloads) {
			this.debugSeq += 1;
			this.subscribe.debugPayload({ sessionId: this.id, seq: this.debugSeq, payload: safeJson(payload) });
		}
	...
	private async afterToolCall(
		...
		this.subscribe.toolResultAdjusted({
			sessionId: this.id,
			toolCallId: context.toolCall.id,
	...
	private async prepareNextTurn(
		...
		this.subscribe.sessionTurnPrepared({ sessionId: this.id, changes });
		return update;
	}
	...
	private requestApproval(request: ToolApprovalRequest, signal?: AbortSignal): Promise<ToolApprovalDecision> {
		return new Promise<ToolApprovalDecision>((resolve) => {
			...
			const settle = (decision: ToolApprovalDecision): void => {
				...
				this.subscribe.toolApprovalResolved({
					sessionId: this.id,
			...
			this.subscribe.toolApprovalRequest({ sessionId: this.id, request });
			this.publishState();
		});
	}
	...
	requestStop(cancel: boolean): void {
		...
		if (this.stopRequested) {
			this.subscribe.sessionStopRequested({ sessionId: this.id, reason: { kind: "user-requested" } });
		}
		this.publishState();
	}
	...
	async configure(patch: AgentConfigPatch): Promise<void> {
		...
		if (patch.skillDirs !== undefined || patch.promptTemplateDirs !== undefined) {
			await this.loadResources();
			this.applySystemPrompt(busy);
			this.subscribe.sessionResources({ sessionId: this.id, resources: this.resources() });
		}
	...
	private async rebindWorkspace(target: string, busy: boolean): Promise<void> {
		...
		this.subscribe.sessionWorkspace({ sessionId: this.id, workspace: target });
		this.subscribe.sessionResources({ sessionId: this.id, resources: this.resources() });
	...
	publishState(): void {
		if (this.closed) return;
		this.subscribe.sessionState(this.snapshot());
	}
	...
	/** Resolve once no run is in flight. Used by close and reset. */
	...
	private setStopReason(reason: StopReason): void {
		...
		this.subscribe.sessionStopRequested({ sessionId: this.id, reason });
	}
	...
	private async applyFold(fold: CompactionFold, automatic: boolean): Promise<void> {
		...
		if (state) {
			this.subscribe.sessionCompacted({ sessionId: this.id, compaction: state, automatic });
		}
		this.publishState();
	}
	...
	private log(level: "info" | "warn" | "error", message: string): void {
		this.subscribe.log({ sessionId: this.id, level, message });
	}
}
