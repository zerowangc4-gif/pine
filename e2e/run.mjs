/**
 * UI smoke: start runtime + Vite if needed, ensure Chromium, run Playwright tour.
 *
 *   DEEPSEEK_API_KEY=sk-... pnpm test:ui
 *
 * Browser install is part of this script (no separate playwright:install).
 */
...
function ensureChromium() {
	console.log("[test:ui] 确保 Playwright Chromium 已安装…");
...
function start(script) {
	...
	child.on("exit", (code, signal) => {
		if (signal !== "SIGTERM" && code && code !== 0) {
			console.error(`[test:ui] ${script} 退出 code=${code}`);
		}
	});
	return child;
}
...
async function main() {
	...
	if (needRuntime) {
		console.log(`[test:ui] 启动 runtime → :${RUNTIME_PORT}`);
		start("dev:runtime");
	} else {
		console.log(`[test:ui] 复用已有 runtime :${RUNTIME_PORT}`);
	}
	if (needUi) {
		console.log(`[test:ui] 启动 UI → :${UI_PORT}`);
		start("dev:ui");
	} else {
		console.log(`[test:ui] 复用已有 UI :${UI_PORT}`);
	}

	await waitForPort(RUNTIME_PORT, "runtime");
	await waitForPort(UI_PORT, "UI");
	console.log(`[test:ui] 服务就绪，开始 Playwright → ${UI_URL}`);

	const tour = spawn("pnpm", ["exec", "tsx", "e2e/deepseek-tour.mjs"], {
		cwd: root,
		stdio: "inherit",
		shell: true,
