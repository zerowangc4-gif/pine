 ...
 * 原则：一旦检查失败立即停止，修好后再测。
 *
 *   DEEPSEEK_API_KEY=sk-... pnpm test:ui
 */
...
// 1×1 PNG for session:prompt images path
writeFileSync(
	join(workspace, "dot.png"),
	Buffer.from(
...
writeFileSync(
	join(workspace, ".pine", "commands", "summarize.md"),
	"Summarize $1 in exactly one short sentence.\n",
	"utf8",
);
...
async function ensureConfigOpen(page) {
	if (
		...
		await page
			.getByRole("button", { name: /配置|Config|展开/ })
			.first()
			.click()
			.catch(() => undefined);
...
async function ensureInspectorOpen(page) {
	...
	if (
		...
		await page
			.getByRole("button", { name: /检查器|Inspector|展开|Show/ })
			.first()
			.click()
			.catch(() => undefined);
...
async function waitConnected(page) {
	await page
		.getByText(/已连接|Connected|已恢复|Recovered/)
		.first()
...
async function waitIdle(page, ms = IDLE_MS) {
	...
	try {
		...
		await page
			.getByRole("button", { name: /立即中断|Abort/ })
			.click()
			.catch(() => undefined);
...
async function waitRunning(page, ms = 20_000) {
	await page
		.getByRole("button", { name: /立即中断|Abort|本轮结束后停止|Stop after/ })
		.first()
		.waitFor({ timeout: ms });
}

async function startLongRun(page) {
	await page.locator("main textarea").last().fill("写四十句关于松树，每句单独一行，尽量写长一些，不要提前结束");
	await clickBtn(page, /^(发送|Send)$/);
	await waitRunning(page);
}

async function queueFollowUp(page, text) {
	await page.locator("main textarea").last().fill(text);
	await clickBtn(page, /排入后续|Follow/);
	await page
		.getByText(/已排队|queued/i)
		.first()
		.waitFor({ timeout: 10_000 });
}

async function queueSteer(page, text) {
	const steer = page.getByRole("button", { name: /^(插话引导|Steer)$/ });
	await steer.first().waitFor({ state: "visible", timeout: 15_000 });
	await page.locator("main textarea").last().fill(text);
...
try {
	...
	await section("A. 壳层", async () => {
		...
		check("已连接", true);
		await page
			.getByRole("button", { name: /收起面板|Hide/ })
			.first()
			.click()
			.catch(() => undefined);
		await page.waitForTimeout(200);
		await page
			.getByRole("button", { name: /展开面板|配置|Config|Show/ })
			.first()
			.click()
			.catch(() => undefined);
		check("配置面板切换", true);
		await page
			.getByRole("button", { name: /检查器|Inspector/ })
			.first()
			.click()
			.catch(() => undefined);
		await page.waitForTimeout(200);
		await page
			...
			.catch(() => undefined);
		await page
			.getByRole("button", { name: /展开|检查器|Show|Inspector/ })
			.first()
			.click()
			.catch(() => undefined);
		check("检查器面板切换", true);
	...
	await section("B. 模型与高级配置字段", async () => {
		...
		check("思考级别映射字段", (await page.getByText(/服务商思考级别映射|Provider thinking-level map/).count()) > 0);
		await toggle(page, "推理型模型", false);
		await apply(page);
	});
	...
	await section("C. 工作区选择器全路径", async () => {
		...
		if (await hidden.count()) {
			...
			check(
				"显示隐藏目录",
				(await dialog.locator("text=.hidden-dir").count()) + (await dialog.locator("text=.pine").count()) > 0,
			);
		...
		check(
			...
			(await page.getByText(/workspace switched|工作区|switched to/i).count()) > 0 ||
				(await page.getByText(workspace.replace(/\\/g, "/").slice(-20)).count()) > 0,
			"切换后应有 log/路径提示",
		);
		await page
			.getByRole("button", { name: /重新加载|Reload/ })
			.first()
			.click()
			.catch(() => undefined);
	});
	...
	await section("D. 会话 + 对话工具", async () => {
		...
		check(
			"tool:resultAdjusted",
			(await page.getByText(/result truncated|truncated to|已截断|截断/i).count()) > 0,
		);
		await fillLabeled(page, "工具结果字节上限", "65536");
		await apply(page);
	});
	...
	await section("E. 审批全分支", async () => {
		...
		if (await always.count()) {
			...
			await page
				.getByRole("button", { name: /允许 write|Allow write|允许一次/ })
				.first()
				.click();
		...
		await page
			.getByRole("button", { name: /^(拒绝|Block)$/ })
			.first()
			.click();
		await waitIdle(page);
		...
		await page
			.getByRole("button", { name: /拒绝并停止|Block and stop/ })
			.first()
			.click();
		await waitIdle(page);
	...
	await section("F. 队列 / 停止 / 中断 / 三种 clearQueue", async () => {
		await startLongRun(page);

		await queueFollowUp(page, "FOLLOW-A");
		check("session:followUp", true);
		await openInspectorTab(page, /状态|State/);
		await page.getByRole("button", { name: /清空后续|Clear follow/ }).first().click();
		await page.waitForTimeout(400);
		check(
			"session:clearQueue followUp",
			(await page.getByRole("button", { name: /清空后续|Clear follow/ }).count()) === 0,
		);

		await queueFollowUp(page, "FOLLOW-B");
		await queueSteer(page, "STEER-B");
		check("session:steer", true);
		await openInspectorTab(page, /状态|State/);
		await page.getByRole("button", { name: /清空插话|Clear steering/ }).first().click();
		await page.waitForTimeout(400);
		check(
			"session:clearQueue steering",
			(await page.getByRole("button", { name: /清空插话|Clear steering/ }).count()) === 0,
		);

		await queueFollowUp(page, "FOLLOW-C");
		await queueSteer(page, "STEER-C").catch(async () => {
			// 若插话按钮被吞，至少再排一条后续保证 clear all 有目标
			await queueFollowUp(page, "FOLLOW-C2");
		});
		await openInspectorTab(page, /状态|State/);
		await page.getByRole("button", { name: /清空全部队列|Clear all queues/ }).first().click();
		await page.waitForTimeout(400);
		check(
			"session:clearQueue all",
			(await page.getByText(/FOLLOW-|STEER-/).count()) === 0 ||
				(await page.getByRole("button", { name: /清空后续|Clear follow/ }).count()) === 0,
		);

		// 再开一轮测 stopRequested 徽章 + abort
		const stillRunning = await page
			.getByRole("button", { name: /立即中断|Abort/ })
			.first()
			.isVisible()
			.catch(() => false);
		if (!stillRunning) await startLongRun(page);
		...
		check("session:requestStop", true);
		await openInspectorTab(page, /状态|State/);
		check(
			"session:stopRequested（状态徽章）",
			(await page.getByText(/本轮结束后停止|Stop after/i).count()) > 0,
		);
		const cancelStop = page.getByRole("button", { name: /取消停止|Cancel stop/ });
		if (await cancelStop.count()) {
			await cancelStop.first().click({ force: true });
			check("requestStop cancel", true);
		}

		const abort = page.getByRole("button", { name: /立即中断|Abort/ });
		await abort.first().waitFor({ state: "visible", timeout: 15_000 });
		await abort.click();
		check("session:abort", true);
	...
	await section("G. 检查器", async () => {
		...
		await page
			.getByRole("button", { name: /清空|Clear/ })
			.first()
			.click()
			.catch(() => undefined);
		...
		await page
			...
			.click();
		await page
			.getByRole("button", { name: "当前工作目录", exact: true })
			.or(page.getByRole("button", { name: "This workspace", exact: true }))
			.first()
			.click();
	...
	await section("H. 压缩 / continue / truncate / setMessages / reset / close", async () => {
		...
		if (await compact.count()) {
			...
			check(
				"session:compact + session:compacted",
				(await page.getByText(/上下文已压缩|Context compacted|折叠/i).count()) > 0,
			);
		} else check("session:compact", false, "按钮不存在");
		...
		if ((await rewind.count()) > 0) {
			...
			await page.waitForTimeout(600);
			check("session:truncate", true);
		} else check("session:truncate", false, "消息不足");

		const continueBtn = page.getByRole("button", { name: /^(继续|Continue)$/ });
		check("继续按钮可见", (await continueBtn.count()) > 0);
		if (await continueBtn.count()) {
			await continueBtn.first().click();
			await waitIdle(page);
			check("session:continue", true);
		} else {
			check("session:continue", false, "无继续按钮");
		}
		...
		if (await msgBox.count()) {
			...
			await page.waitForTimeout(600);
			check("session:setMessages 清空", true);
			await msgBox
				.first()
				.fill(JSON.stringify([{ role: "user", content: "setMessages-ok", timestamp: Date.now() }], null, 2));
			await clickBtn(page, /应用消息列表|Apply messages/);
			await page.waitForTimeout(800);
			check("session:setMessages 写入", (await page.getByText("setMessages-ok").count()) > 0);
		} else check("session:setMessages", false, "未找到 JSON 编辑框");

		await clickBtn(page, /清空对话|Clear transcript/);
		await page.waitForTimeout(600);
		check("session:reset", true);
		...
		await page.waitForTimeout(600);
		check("session:close", (await page.getByRole("button", { name: /开始会话|Start/ }).count()) > 0);
	...
	await section("I. turnPrepared / 图片 prompt / 断线重连", async () => {
		await clickBtn(page, /开始会话|新建会话|Start|New/).catch(() => undefined);
		await page.waitForTimeout(600);

		await startLongRun(page);
		await ensureConfigOpen(page);
		await fillLabeled(page, "系统提示词", `You are Pine. TurnPrepared marker ${Date.now()}. Prefer short answers.`);
		await apply(page);
		await waitIdle(page, IDLE_MS);
		check(
			"session:turnPrepared",
			(await page.getByText(/已在轮次边界生效|Applied at the turn boundary/i).count()) > 0,
		);

		await ensureConfigOpen(page);
		await toggle(page, "支持图片输入", true);
		await apply(page);
		const attach = page.locator('input[type="file"][accept*="image"]');
		if (await attach.count()) {
			await attach.first().setInputFiles(join(workspace, "dot.png"));
			await page.waitForTimeout(400);
			await page.locator("main textarea").last().fill("若能看到图片请只回 IMG-OK，否则只回 IMG-DROP");
			await clickBtn(page, /^(发送|Send)$/);
			await waitIdle(page);
			check(
				"session:prompt(+images)",
				(await page.getByText(/IMG-OK|IMG-DROP|image attachment|dropped|图片/i).count()) > 0 ||
					(await page.locator("main img").count()) > 0,
			);
		} else {
			check("session:prompt(+images)", false, "无附件控件");
		}
		await toggle(page, "支持图片输入", false);
		await apply(page);

		const disconnected = await page.evaluate(() => {
			const client = globalThis.__pine;
			if (!client?.disconnect) return false;
			client.disconnect();
			return true;
		});
		check("触发断线", disconnected, disconnected ? "" : "无 window.__pine（需 DEV）");
		if (disconnected) {
			await page
				.getByRole("button", { name: /立即重连|Reconnect/ })
				.first()
				.waitFor({ timeout: 10_000 });
			await clickBtn(page, /立即重连|Reconnect/);
			await waitConnected(page);
			check("立即重连", true);
		}
	});
